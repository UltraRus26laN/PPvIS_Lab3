...
{
    private final Group group = new Group(); private Integer x = 0;
    private Integer limit; private Integer DoubleNumberN; private Integer n = 1;
    ...
    {
        setNumberTempLine(graphicGroup); setArrTempLine(graphicGroup);
        ...
        startBuildButton.setOnAction(actionEvent ->
        {
            ...
            if (!numberCalculationThread.isAlive())
            {
                if (inputCheck(NumberN.getText()) && inputCheck(maxLimitForX.getText()))
                {
                    ...
                    numberQueue.clear(); table.clearTable();
                    ...
                    {
                        arrQueue.clear(); graphicGroup.createNewNumberSeriesForCoplexFunction(limit);
        ...
        stopBuildButton.setOnAction(actionEvent ->
        {
            if (!numberCalculationThread.isInterrupted())
            {
                ...
                numberQueue.clear(); arrQueue.clear();
            }
        });
    ...
    private void setNumberTempLine(ChartGroup chartGroup)
    {
        ...
        numberTempLine.getKeyFrames().add(new KeyFrame(Duration.millis(100), actionEvent ->
        { if (!numberQueue.isEmpty()) { chartGroup.updateNumberSeriesList(x, numberQueue.poll()); x += 1; } }));
        numberTempLine.play();
    }
    ...
    {
        boolean isInteger = false; String numberMatcher = "^-?[0-9]*$";
        if (!text.isEmpty()) { if (!text.matches(numberMatcher)) { errorAlert(); } else { isInteger = true; } }
        return isInteger;
    }
    ...
    private void setArrTempLine(ChartGroup chartGroup)
    {
        ...
        wordTempLine.getKeyFrames().add(new KeyFrame(Duration.millis(100), actionEvent ->
        { if (!arrQueue.isEmpty()) { chartGroup.updateWordSeriesList(n, arrQueue.poll()); n += 1; } }));
        wordTempLine.play();
    }
}